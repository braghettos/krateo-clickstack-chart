...
|------|-------|--------------|---------|
| `charts/clickstack` | `clickstack` | `oci://ghcr.io/braghettos/krateo/clickstack` | ClickStack (ClickHouse + OTel gateway + HyperDX) wrapper with `values.schema.json`, `/events` http-handlers and configurable HyperDX exposure |
...
instance. The collectors depend on the `clickstack` composition (for the ClickHouse credentials
Secret), and `krateo-sse-proxy` is exposed so the portal's events bell can reach it.

## Local validation

```sh
helm lint charts/clickstack
helm template smoke charts/clickstack
```
